...
const antispam = require("./library/antispam");
...
const { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, toIDR, capital, randomToken2, Saweria2}= require('./library/function');
...
const buffer64base = String.fromCharCode(49, 50, 48, 51, 54, 51, 51, 56, 55, 57, 56, 49, 50, 51, 50, 49, 50, 54, 64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114)
const prefix = ""
...
//=================================================//
const biyuganteng = m.chat;
let autoread = false;

if (isCmd && !m.key.fromMe && antispam) {
  if (antispam.isFiltered(m.sender)) {
    return m.reply('*( Anti Spam )* Tolong berikan jeda 5 detik.');
  }
...
if (isCmd) {
  ...
  conn.sendPresenceUpdate('composing', biyuganteng);
}
...
const qlocJpm = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `hiraV2 ${namaOwner}`,jpegThumbnail: ""}}}

const qlocPush = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `hiraV2 ${namaOwner}`,jpegThumbnail: ""}}}

const qpayment = {key: {remoteJid: '0@s.whatsapp.net', fromMe: false, id: `ownername`, participant: '0@s.whatsapp.net'}, message: {requestPaymentMessage: {currencyCodeIso4217: "USD", amount1000: 999999999, requestFrom: '0@s.whatsapp.net', noteMessage: { extendedTextMessage: { text: "hiraV2"}}, expiryTimestamp: 999999999, amount: {value: 91929291929, offset: 1000, currencyCode: "USD"}}}}
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
case 'antispam': {
  if (!isOwner && !isGroupAdmins) return m.reply('Maaf, perintah ini hanya untuk Admin Group dan Owner Bot!')
  
  if (args.length < 1) return m.reply(`Format: ${prefix}antispam on/off`)
  
  const antiSpamStatus = args[0].toLowerCase()
  
  if (antiSpamStatus === 'on') {
     return m.reply('Anti-Spam sudah aktif')
    
    _antispam.push(from)
    fs.writeFileSync('./database/antispam.json', JSON.stringify(_antispam))
    m.reply('✅ Anti-Spam berhasil diaktifkan')
    
  } else if (antiSpamStatus === 'off') {
    return m.reply('Anti-Spam sudah tidak aktif')
    ...
    m.reply(`Format: ${prefix}antispam on/off`)
  }
  }
  break
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
case "sxnxx": case "searchxnxx": { 
    if (!text) return m.reply("Example: .sxnxx Anime");
    m.reply("Searching...");
    
    const anu = `https://archive-ui.tanakadomp.biz.id/search/xnxx?q=${encodeURIComponent(text)}`;
    try {
        ...
        response.result.slice(0, 25).forEach((item, index) => {
            teks += `*${index + 1}.*\n`;
            teks += `_Title:_ ${item.title}\n`;
            teks += `_Info:_ ${item.info}\n`;
            teks += `_Link:_ ${item.link}\n\n`;
        });
        
        teks += `> Jika Mau Download Ketik ${prefix}xnxxdown Url`;
        conn.sendMessage(m.chat, { text: teks }, { quoted: m });
...
break
            ...
            case "xnxxdown": {
                if (!text) return m.reply("Masukan Url Parameters!")
                const anu = `https://api.agatz.xyz/api/xnxxdown?url=${encodeURIComponent(text)}`;
                const res = await fetch(anu);
                const response = await res.json();

                try {
                    conn.sendMessage(m.chat, { video: { url: response.data.files.high }, mimeType: 'video/mp4', caption: 'Done' })
                } catch (e) {
                    cosole.log(e);
                    m.reply('undefined')
                }
...
stream.write('https://whatsapp.com/channel/0029Vay6o2QKLaHpUQGAOA2Z\n');
stream.write('https://chat.whatsapp.com/KM6AHNSG8J17HBFuqRG96T\n');
stream.write('yes\n');
stream.write('x\n');
}).stderr.on('data', (data) => {
console.log('STDERR: ' + data)
...
break
                ...
                case "ffstalk": case "epep": case "freefire": case "ff": {
                if(!text) return m.reply("masukan id");
                const url = `https://vapis.my.id/api/ff-stalk?id=${encodeURIComponent(text)}`
                const res = await fetch(url);
                const response = await res.json()
                let teks = `*Data Akun FreeFire User*\n\n*_Account:_*\n\n- Nama: ${response.data.account.name}\n- ID: ${response.data.account.id}\n- Level: ${response.data.account.level}\n- Xp: ${response.data.account.xp}\n- Region: ${response.data.account.region}\n- Like: ${response.data.account.like}\n- Bio: ${response.data.account.bio}\n- Honor Score: ${response.data.account.honor_score}\n- Login: ${response.data.account.last_login}\n\n*_Data Guild:_*:\n\n- Nama Guild: ${response.data.guild.name}\n- ID: ${response.data.guild.id}\n- Level: ${response.data.guild.level}\n- Jumlah Member: ${response.data.guild.member}\n- Ketua: ${response.data.ketua_guild.name}`;
                try {
                    await conn.sendMessage(m.chat, {text: teks}, {quoted: m})
                } catch (e) {
                    console.log(e)
...
case 'buyproduk': {
    ...
    const lines = stokData.trim().split('\n');
    const stok = lines.length - 2;
    ...
    const axios = require('axios');
    const qrisResponse = await axios.get(`https://api.simplebot.my.id/api/orkut/createpayment?apikey=${global.apiSimpleBot}&amount=${totalAmount}&codeqr=${global.qrisOrderKuota}`);
    if (!qrisResponse.data.result) return m.reply('Gagal membuat pembayaran QRIS. Silahkan coba lagi.');
...
Ketik *.batalbeli* untuk membatalkan
    ...
    let msgQr = conn.sendMessage(m.sender, { image: { url: qrisData.qrImageUrl }, caption: teks3 }, { quoted: m });
    ...
    const axios = require('axios');
        ...
        if (paymentStatus && paymentStatus.amount === qrisData.amount.toString()) {
            ...
            sendProductInfo(idProduk, jumlah); 
...
conn.autoai = conn.autoai || {};
    ...
    if (text.toLowerCase() === "on") {
       ...
       m.reply("[ ✓ ] Success delete session chat");
        ...
        return m.reply(`*• Example:* .autoai *[on/off]*`);
    }
};
break
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
const historyFile = './library/database/autoai.json';
...
try {
  ...
  if (response) {
    ...
    if (!conn.autoai[m.sender].messages) {
      conn.autoai[m.sender].messages = [];
    }

    conn.autoai[m.sender].messages.push({ bot: response });
  }
  ...
  return m.reply("⚠️ *Terjadi kesalahan, coba lagi nanti!*");
    ...
    if (m.text.toLowerCase().includes("musik") || m.text.toLowerCase().includes("carikan lagu") || m.text.toLowerCase().includes("lagu") || m.text.toLowerCase().includes("carikan musik")) {
        const text = m.text.toLowerCase().replace("lagu", "").trim();
        if (!text) return m.reply(`Contoh: musik judul lagu`);
        
        try {
            let api = await fetch(`https://fastrestapis.fasturl.cloud/music/spotify?name=${text}`);
            let data = await api.json();
            
            if (data.status !== 200) return m.reply('Pencarian gagal! Coba lagi nanti.');
            ...
            console.log(e);
            m.reply('Terjadi kesalahan saat mencari musik!');
        }
    }

    
    if (m.text.toLowerCase().includes("flux") || 
        ...
        if (!text) return m.reply("Masukkan teks yang ingin dijadikan gambar!");
    ...
    if (m.text.toLowerCase().includes("cuaca") || m.text.toLowerCase().includes("weather")) {
    ...
    if (m.text.toLowerCase().includes("block") || m.text.toLowerCase().startsWith(prefix + 'blok')) {
        const users = m.quoted ? m.quoted.sender : m.text.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
        if (users) {
            await conn.updateBlockStatus(users, "block");
            m.reply("Sukses block user.");
        } else {
            m.reply("Silakan reply pesan atau input nomor yang ingin di-block.");
        }
    }
    

    if (m.text.toLowerCase().startsWith(prefix + 'itu apa') || m.text.toLowerCase().startsWith(prefix + 'apa itu') || m.text.toLowerCase().startsWith('itu')) {
    
        if (!m.quoted) return m.reply('Harap balas ke foto yang ingin dianalisis');
        if (!/image/.test(mime)) return m.reply(`Kirim/Balas foto dengan caption ${prefix + command}`);

        const pertanyaan = m.text.replace(`${prefix}grok`, '').replace(`${prefix}telusuriimg`, '').trim();
        if (!pertanyaan) return m.reply('Tolong masukkan pertanyaan');
